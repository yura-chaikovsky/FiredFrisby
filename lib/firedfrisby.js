...
var globalSetup = function (opts = {}) {
    ...
    if (!_.isUndefined(opts)) {
        // check if global setup has already been performed once and complain if the user is providing another global
        // setup (not deep equal)
        ...
        _.merge(_getDefaultConfig, opts);
    }
};
...
function Frisby(msg) {
    ...
    var config = _.clone(_getDefaultConfig, true);
...
Frisby.prototype.reset = function () {
    this.current.request = _.cloneDeep(_getDefaultConfig.request);
    return this;
};
...
Frisby.prototype._request = function (/* method [uri, data, params] */) {
    ...
    if (!outgoing.json && data && (outgoing.headers['content-type'].indexOf('json') > -1)) {
        console.warn('You specified a content-type header with \'json\' but did not specify the body type to be json.');
    }
...
// Debugging helper to inspect full HTTP message
Frisby.prototype.inspectAll = function () {
    this.inspect(
        function (err, req, res, body) {
            let request = {
                METHOD: req.method,
            ...
            console.log(util.inspect(request,  {depth: null, colors: true, breakLength: 0 }));
            ...
            console.log(util.inspect(response,  {depth: null, colors: true, breakLength: 1 }), "\n\n");
        });
    return this;
...
Frisby.prototype.toss = function (retry) {
    ...
    describe(self.current.describe, function () {
        it("\n\t[ " + self.current.itInfo + " ]", function (done) {
            ...
            // launch request
            // repeat request for self.current.retry times if request does not respond with self._timeout ms (except
            // for POST requests)
            var tries = 0;
            ...
            function assert() {
                ...
                // Some 'expects' helpers add more tests when executed (recursive 'expectJSON' and 'expectJSONTypes',
                // with nested JSON syntax etc.)
